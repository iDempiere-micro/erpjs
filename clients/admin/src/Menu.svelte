<script lang='ts'>
    ...
    const EVERYTHING = gql`
        { menu { id displayName items { id to displayName} }}
    `;
...
<div class="ml-10 flex items-baseline space-x-4">
    {#each ($menuStore?.data?.menu[0].items)||[] as menuItem}
        <Link
            to={`/${menuItem.to}`}
            class=
                {segment === menuItem.to ? "bg-gray-900 text-white px-3 py-2 rounded-md text-sm font-medium" : "text-gray-300 hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md text-sm font-medium"}
